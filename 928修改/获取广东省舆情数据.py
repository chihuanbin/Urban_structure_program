...
url_amap = 'http://api.map.baidu.com/place/v2/search?query=学校&page_size=20&page_num={page_index}&scope=2&region=广州市&coord_type=3&output=json&ak=8VUjfqGwgnHwEZLxwPpnZvO1Sgeq2HFO'
...
hkeys=['id', '名称', '详情网址', '地址', '联系电话', '北纬', '东经']
bkeys = ['uid', 'name', ['detail_info','detail_url'], 'address', 'telephone', ['location', 'lat', 'lng']]
# 获取数据列
#bkeys = ['uid', 'name', ['detail_info', 'tag', 'detail_url'], 'address', 'telephone', ['location', 'lat', 'lng']]
#fd8ac4def17b154976d096a2b784da80高德地图秘钥
u"""
        城市内检索
...
def get_data(pageindex):
    ...
    url=url_amap.format(page_index=str(pageindex))
    # 中文编码
    url = quote(url, safe='/:?&=')
    html = ""
    print(page_index)
...
if __name__ == '__main__':
    ...
    getPOIdata()
    # 读取json文件数据写入到excel
    write_data_to_excel("广东省广州市学校-百度地图纠偏")